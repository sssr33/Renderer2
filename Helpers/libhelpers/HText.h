...
#include "PlatformStringBuilder.h"
...
public:
	...
	template<class T> static T ConvertUTF8(const std::string &utf8) {
		static_assert("Not Implemented", false);
	}

	template<> static std::wstring ConvertUTF8(const std::string &utf8) {
#ifdef HAVE_VISUAL_STUDIO
		std::wstring wpath;
		// Runtime is modern, narrow calls are widened inside CRT using CP_ACP codepage.
		UINT codepage = CP_UTF8;

		auto length = MultiByteToWideChar(codepage, 0, utf8.data(), (int)utf8.length(), nullptr, 0);

		wpath.resize(length);

		MultiByteToWideChar(codepage, 0, utf8.data(), (int)utf8.length(), &wpath[0], (int)wpath.size());
#elif
#error Add your implementation
	...
	static std::string ConvertToUTF8(const std::wstring &s){
		std::string result;
#ifdef HAVE_VISUAL_STUDIO
		UINT codepage = CP_UTF8;

		int requiredLength = WideCharToMultiByte(codepage, 0, s.data(), (int)s.size(), nullptr, 0, nullptr, nullptr);
...
#error Add your implementation
	...
	static std::string ConvertToUTF8(Platform::String ^v){
		...
		int requiredLength = WideCharToMultiByte(codepage, 0, v->Data(), (int)v->Length(), nullptr, 0, nullptr, nullptr);
	...
	template<> static Platform::String ^ConvertUTF8(const std::string &utf8) {
		PlatformStringBuilder builder;
		// Runtime is modern, narrow calls are widened inside CRT using CP_ACP codepage.
		UINT codepage = CP_UTF8;

		auto length = MultiByteToWideChar(codepage, 0, utf8.data(), (int)utf8.length(), nullptr, 0);

		builder.Resize((uint32_t)length);

		MultiByteToWideChar(codepage, 0, utf8.data(), (int)utf8.length(), builder.GetData(), (int)length);
